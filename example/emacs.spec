...
%endif
# default is without nativecomp always
%bcond_with     nativecomp
%bcond_with     nativecompaot
%bcond_without  cairo
%bcond_with     tex4pdf
%bcond_with     memmmap
...
Provides:       epg = 1.0.0
Obsoletes:      epg < 1.0.0
Requires:       emacs-info%{customcc_pkg_suffix} = %{version}
...
%package     -n emacs-nox%{customcc_pkg_suffix}
Requires(post): fileutils
Requires:       emacs%{customcc_pkg_suffix} = %{version}-%{release}
%if %{with nativecomp}
Requires:       emacs-eln%{customcc_pkg_suffix} = %{version}
%endif
Provides:       emacs_program = %{version}-%{release}
Summary:        GNU Emacs-nox: An Emacs Binary without X Window System Support
Group:          Productivity/Text/Editors

%description -n emacs-nox%{customcc_pkg_suffix}
...
%package     -n emacs-x11%{customcc_pkg_suffix}
Requires(post): fileutils
Requires:       emacs%{customcc_pkg_suffix} = %{version}-%{release}
%if %{with nativecomp}
Requires:       emacs-eln%{customcc_pkg_suffix} = %{version}
...
%description -n emacs-x11%{customcc_pkg_suffix}
Call it
...
%if %{with nativecomp}
%package     -n emacs-eln%{customcc_pkg_suffix}
...
Requires:       emacs-el%{customcc_pkg_suffix} = %{version}
Summary:        GNU Emacs-nox: Emacs Lisp native compiled binary files
Group:          Productivity/Text/Editors

%define _libeln %{_prefix}/lib

%description -n emacs-eln%{customcc_pkg_suffix}
...
%package     -n emacs-el%{customcc_pkg_suffix}
...
%description -n emacs-info%{customcc_pkg_suffix}
...
%package     -n etags%{customcc_pkg_suffix}
...
%description -n etags%{customcc_pkg_suffix}
...
%endif
%if %{with nativecomp}
%if %{with nativecompaot}
NATIVECOMP_AOT="NATIVE_COMPILATION_AOT=yes"
%else
NATIVECOMP_AOT="NATIVE_COMPILATION_AOT=no"
%endif
%else
NATIVECOMP_AOT=
%endif
...
CFLAGS="$CFLAGS -DPDMP_BASE='\"emacs-nox\"'" ./configure ${NATIVECOMP_AOT} ${COMP} ${PREFIX} ${NOX11} ${SYS} --with-dumping=pdumper
%make_build V=1
...
make distclean
#
CFLAGS="$CFLAGS -DPDMP_BASE='\"emacs-gtk\"'" ./configure ${NATIVECOMP_AOT} ${COMP} ${PREFIX} ${GTK} ${SYS} --with-dumping=pdumper
%make_build
...
make distclean
#
CFLAGS="$CFLAGS -DPDMP_BASE='\"emacs-x11\"'" ./configure ${NATIVECOMP_AOT} ${COMP} ${PREFIX} ${X11} ${SYS} --with-dumping=pdumper
%make_build
...
%post -n emacs-nox%{customcc_pkg_suffix}
...
%post -n emacs-x11%{customcc_pkg_suffix}
if test -e usr/share/emacs/site-lisp/auctex/font-latex.elc ; then
  owd=$(pwd)
  cd usr/share/emacs/site-lisp/auctex || exit 1
  emacs -batch -no-site-file -no-init-file --eval '(setq load-path (cons "." load-path)
...
%if 0%{?suse_version} <= 1500
%post -n emacs-info%{customcc_pkg_suffix}
...
%preun -n emacs-info%{customcc_pkg_suffix}
for f in %info_files; do
  test "$f" = "info.info" && continue
  test -e "$f" || f="${f}.info"
...
%post -n etags%{customcc_pkg_suffix}
...
%postun -n etags%{customcc_pkg_suffix}
...
%files       -n emacs-nox%{customcc_pkg_suffix}
%defattr(-, root, root)
%{_bindir}/emacs-nox
%{_libexecdir}/emacs/%{version}/*-suse-linux*/emacs-nox.pdmp

%files       -n emacs-x11%{customcc_pkg_suffix}
%defattr(-, root, root)
%{_bindir}/emacs-x11
%{_bindir}/emacs-gtk
...
%if %{with nativecomp}
%files       -n emacs-eln%{customcc_pkg_suffix} -f eln.list
%defattr(-, root, root)
%dir %{_libeln}/emacs/
...
%files       -n emacs-info%{customcc_pkg_suffix}
%defattr(-, root, root)
...
%files       -n emacs-el%{customcc_pkg_suffix}
%defattr(-, root, root)
...
%files -n etags%{customcc_pkg_suffix}
%defattr(-,root,root)
%doc etc/ETAGS.EBNF
